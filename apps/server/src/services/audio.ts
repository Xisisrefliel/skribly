...
/**
 * Process audio file: convert to optimal format and split into chunks
...
export async function processAudioFile(
  ...
  console.log(`Processing audio: ${originalFilename}, duration: ${Math.round(totalDuration)}s`);

  // If audio is short enough (under 4 minutes), no need to split
  // Just convert to optimal format
  if (totalDuration <= CHUNK_DURATION_SECONDS * 0.8) {
    const convertedPath = path.join(tempDir, 'converted.mp3');
    await convertToOptimalFormat(inputPath, convertedPath);
  ...
  const chunks = await splitAudioIntoChunks(inputPath, chunksDir);
